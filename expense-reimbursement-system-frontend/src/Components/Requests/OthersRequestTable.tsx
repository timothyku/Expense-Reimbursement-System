...
export const OthersRequestTable: React.FC = () => {
  ...
  const fetchRequests = async (endpoint: string) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8080/requests/${endpoint}`,
  ...
  const handleRequestAction = async (
    ...
    await axios.post(
      `http://localhost:8080/requests/${action}`,
      { reimbId: requestId },
      { withCredentials: true }
    );
    getOthersPendingRequests();
      ...
      <Navbar bg="dark" variant="dark" expand="lg" className="shadow-sm px-3">
        ...
        <Nav className="ms-auto">
          ...
          <NavDropdown title="Navigation" id="nav-dropdown" menuVariant="dark">
            ...
            <NavDropdown.Item
              onClick={() => navigate("/requests/my-requests-manager")}
            >
              My Requests
            </NavDropdown.Item>
          </NavDropdown>
      ...
      <Container className="d-flex flex-column align-items-center mt-4">
        ...
        >
          ...
          <div className="d-flex justify-content-center gap-2 mb-3">
            <Button
              variant={viewAll ? "outline-secondary" : "dark"}
              onClick={getOthersPendingRequests}
            >
              <FaClipboardList className="me-2" /> Pending Requests
            </Button>
            <Button
              variant={viewAll ? "dark" : "outline-secondary"}
            ...
            <Table hover responsive className="text-center">
              ...
              <tbody>
                {requests.length > 0 ? (
                  requests.map((req) => (
                    <tr key={req.reimbId}>
                      <td style={{ verticalAlign: "middle" }}>{req.reimbId}</td>
                      ...
                      {!viewAll && (
                        ...
                        >
                          ...
                          <Button
                            ...
                            onClick={() =>
                              handleRequestAction(req.reimbId, "deny")
                            }
                          >
                            Deny
                          </Button>
                  ...
                  <tr>
                    <td colSpan={viewAll ? 5 : 6} className="text-muted py-3">
                      No requests found.
                    </td>
                  </tr>
                )}
